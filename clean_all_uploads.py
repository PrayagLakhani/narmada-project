...
# Create clean templates for each upload page
pages = [
    ('admin', 'testing', '/api/admin-ingest-testing'),
    ('admin', 'training', '/api/admin-ingest-training'),
    ('collaborator', 'display', '/api/collaborator-update-stations-display'),
    ('collaborator', 'testing', '/api/collaborator-ingest-testing'),
    ('collaborator', 'training', '/api/collaborator-ingest-training'),
]

for role, page_type, api_endpoint in pages:
    page_title_map = {
        'display': 'Display',
        'testing': 'Testing',
        'training': 'Training'
    ...
    title = f"Upload {page_title_map[page_type]} Data"
...
<main class="flex-grow flex items-center justify-center p-6">
  <form id="uploadForm" class="w-full max-w-md bg-white rounded-lg shadow-sm border border-slate-200 p-8">

    <h2 class="text-2xl font-semibold text-slate-900 mb-6">{title}</h2>
    ...
    <p id="status" class="mt-6 text-sm font-medium text-center text-slate-600"></p>
...
print("All upload pages cleaned!")
