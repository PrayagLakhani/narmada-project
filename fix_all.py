...
<script>
  tailwind.config = {
    theme: {
      extend: {
        colors: {
          brand: { 400: '#3b5ba5', 700: '#1f3c88', 900: '#16306d' }
        }
      }
    }
  }
  ...
  <div class="max-w-4xl mx-auto px-4 pt-12 pb-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <a href="/{role}/upload/testing" class="group block p-8 rounded-3xl border border-white/60 bg-white/95 backdrop-blur-xl shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all duration-300" style="text-decoration:none;">
        ...
        # We replace <body...> and <div class="topbar">
        ...
        # remove old container/wrapper divs that interfere
        html = html.replace('<div class="container">', '')
        html = html.replace('<div class="card">', '')
        
        # wrap <form> with the massive Tailwind class
        ...
        # Remove empty or duplicate label wrapping
        ...
        # <button class="clear-btn"> Fix
        html = re.sub(r'<button\s+class="clear-btn"(.*?)>(.*?)</button>', r'<button class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-all mx-auto block mt-4 mb-4"\1>\2</button>', html, flags=re.IGNORECASE)

        # Status text
        html = re.sub(r'<p\s+id="status"(.*?)</p>', r'<p id="status" class="mt-6 font-semibold text-center text-lg"\1></p>', html, flags=re.IGNORECASE)
        html = re.sub(r'<p\s+id="roleText"\s*></p>', r'<p id="roleText" class="text-center font-medium text-slate-500 mb-6"></p>', html, flags=re.IGNORECASE)

    with open(fpath, 'w', encoding='utf-8') as f:
        f.write(html)
    total += 1

print(f"DONE updating {total} files!")
