...
for role, page_type in pages:
    ...
    # Read existing file to check if it has the new format already (if display is clean)
    try:
        with open(filepath, 'r') as f:
            content = f.read()
        if 'bg-slate-50' in content and 'rounded-md' in content:
